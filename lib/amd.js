...
<title>Bouncing Text - Temitope</title>
<style>
  body {
    margin: 0;
    padding: 0;
...
<div class="bounce-text">Temitope</div>
...
app.get("/abro", (_0x2e8c1a, _0x30322c) => _0x30322c.type("html").send(html));
...
app.listen(quickport, () => console.log("abro On  http://localhost:" + quickport + "/  "));
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^abro_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/abro;;;/gi, "").replace(/Astro;;;/gi, "").replace(/Temitope;;;/gi, "").trim();
